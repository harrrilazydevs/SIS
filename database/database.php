...
class db {
    ...
    function get_( $sql , $val =[]){ 
        ...
        ( $stmt ? ( $stmt->execute($val) ? ( !empty($result = $stmt->fetchAll()) ? $status = $result : $status = 403 ) : $status = 500 ) : $status = 500 ); 
        
        return $status; 
    }
    
    function post_( $sql , $val =[]){ 

     /*    var_dump($val); var_dump($sql);  */

        $status = ''; 
        
        $stmt = $this->connect()->prepare($sql); 
        
        ($stmt) ? ($stmt->execute($val)) ? $status = 200 : $status = 500 : $status = 500 ; 
