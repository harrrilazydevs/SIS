...
//  PROGRESS
function progress(id,value){ var progress = document.getElementById(id); progress.innerText = value+'%'; if(value == 0) { progress.style.color = "#000"; progress.style.paddingLeft = "0"; } else { var counter = 0; var count = 4; var loading = setInterval(animate, 10); function animate(){ if(count == 30) { clearInterval(loading); } else { progress.style.color = "white"; progress.style.background = "#007BFF"; progress.style.paddingLeft = "3rem"; while (counter < value){ counter = counter+1; progress.style.width =  counter+ '%'; percent.textContent = counter + '%'; } } } } }
